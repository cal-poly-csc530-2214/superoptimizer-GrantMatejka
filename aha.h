#define OFILE "aha.out"

/** 
   HOW TO USE:

   userfun must be a function with one or two int arguments
   and must return a 32-bit int result

   userfun will then be the function used to find the optimal code
   ...
   // To find Dietz's formula for the floor-average of two unsigned integers.
   int userfun(int x, int y) {     
      return ((unsigned long long)x + (unsigned long long)y) >> 1;
   }
 */

#define DEBUG 0         // 0 or 1; debugging printouts if 1.
#define COUNTERS 0      // 0 or 1; count number of evaluations.
...
#define NBSM 63 // Shift mask.  Use 63 for mod 64 shifts, or 31 for mod 32.
...
// Code below sets this to 1 for an unacceptable operation, such as divide by 0. It is initially 0.
int unacceptable = 0; 

// Collection of simulator routines for the instructions in the isa.
int neg(int x, int _y, int _z) { return -x; }
int _not(int x, int _y, int _z) { return ~x; }
int pop(int xx, int _y, int _z)
{
   unsigned x = xx;
   x = x - ((x >> 1) & 0x55555555);
...
int add(int x, int y, int _z) { return x + y; }
int sub(int x, int y, int _z) { return x - y; }
int mul(int x, int y, int _z) { return x * y; }
...
int _and(int x, int y, int _z) { return x & y; }
int _or(int x, int y, int _z) { return x | y; }
int _xor(int x, int y, int _z) { return x ^ y; }
int rotl(int x, int y, int _z)
{
   int s = y & NBSM;
   return x << s | (unsigned)x >> (32 - s);
}
int shl(int x, int y, int _z)
{
   int s = y & NBSM;
   if (s >= 32)
      return 0;
...
int cmpeq(int x, int y, int _z) { return x == y; }
int cmplt(int x, int y, int _z) { return x < y; }
int cmpltu(int x, int y, int _z) { return (unsigned)(x) < (unsigned)(y); }
